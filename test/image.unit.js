...
describe("image.js", function () {
    describe("imageUrl()", function () {
        it("returns a random image url from loremflickr", function () {
            var imageUrl = faker.image.imageUrl();

            assert.equal(imageUrl, 'http://loremflickr.com/640/480');
        });
        it("returns a random image url from loremflickr with width and height", function () {
            var imageUrl = faker.image.imageUrl(100, 100);

            assert.equal(imageUrl, 'http://loremflickr.com/100/100');
        });
        it("returns a random image url for a specified category", function () {
            var imageUrl = faker.image.imageUrl(100, 100, 'abstract');

            assert.equal(imageUrl, 'http://loremflickr.com/100/100/abstract');
        });
        /*
        it.only("returns a random image url from loremflickr with a randomizer", function () {
            var imageUrl = faker.image.imageUrl(100, 100, undefined, true);

            console.log(imageUrl);
            assert.ok(imageUrl.match(/^http:\/\/loremflickr.com\/100\/100\?[\d]+$/));
        });
        */
    });
